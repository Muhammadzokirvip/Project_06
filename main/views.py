...
def create_categoriya(request):
    if request.method == 'POST':
        models.Category.objects.create(
            name=request.POST['name']
        )
        return redirect('categoriya')
    return render(request, 'dashboard/categoriya/create.html')


def categoriya(request):
    categoriya = models.Category.objects.all()
    return render(request, 'dashboard/categoriya/list.html', {'categoriya':categoriya})



def category_update(request, id):
    categoriya = models.Category.objects.get(id=id)
    if request.method == 'POST':
        categoriya.name = request.POST['name']
        categoriya.save()
        return redirect('categoriya')
    return render(request, 'dashboard/categoriya/update.html', {'categoriya':categoriya})


def categoriya_delete(request, id):
